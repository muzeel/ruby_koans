...
class AboutRegularExpressions < Neo::Koan
  ...
  def test_the_left_most_match_wins
    assert_equal "a", "abbccc az"[/az*/] # ! ищет первое совпадение
  end
  ...
  def test_slash_a_anchors_to_the_start_of_the_string
    ...
    assert_equal nil, "start end"[/\Aend/] # \A - совпадает с началом строки
  end

  def test_slash_z_anchors_to_the_end_of_the_string
    assert_equal "end", "start end"[/end\z/] # \z - совпадает с концом строки
    assert_equal nil, "start end"[/start\z/]
  end

  def test_caret_anchors_to_the_start_of_lines
    assert_equal "2", "num 42\n2 lines"[/^\d+/] # ^ - начало строки
  end

  def test_dollar_sign_anchors_to_the_end_of_lines
    assert_equal "42", "2 lines\nnum 42"[/\d+$/] # $ - конец строки
  end

  def test_slash_b_anchors_to_a_word_boundary
    assert_equal "vines", "bovine vines"[/\bvine./] # \b = граница 
  end
